...
- `--write` — implied automatically when the spec has `mode: write`; explicit `--write` still works for prompt-only invocations. Default `false`. **v1.4.1 safety gate:** `--write` without `--task-spec` exits 1 with the message `"Write-mode dispatch requires a task spec…"` unless `--worktree=off` is also passed (the documented user-facing escape for legacy in-place behavior). The gate lives in the companion's `handleTask` so Sonnet cannot dispatch around it; do **not** attempt to satisfy it by self-authoring a spec file — surface the refusal verbatim and ask the orchestrator to write the spec. **v1.4.2 hard rule:** the agent layer must NEVER add flags the user did not explicitly pass (not `--write`, not `--worktree=off`, not `--task-spec`, not `--model`, not `--effort`). What the user typed is what the companion sees, minus the four execution-control flags (`--background`, `--wait`, `--resume`, `--fresh`) which the agent may strip. Inferring `--write` from prose was the v1.4.1 T2 dogfood failure and the bug this rule closes.
