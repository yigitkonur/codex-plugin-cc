...
- `--write` — implied automatically when the spec has `mode: write`; explicit `--write` still works for prompt-only invocations. Default `false`. **v1.4.3 safety gate (unconditional):** `--write` without `--task-spec` exits 1 with the message `"Write-mode dispatch requires a task spec…"`. There is no escape. The v1.4.1 release of this gate originally honored `--worktree=off` as a documented escape, but the v1.4.1/v1.4.2 dogfoods proved the agent layer abused that escape by self-adding both `--write` and `--worktree=off` together; v1.4.3 retired the escape entirely. The only path to write-mode dispatch is through `--task-spec`. **v1.4.2 hard rule (still in force):** the agent layer must NEVER add flags the user did not explicitly pass. What the user typed is what the companion sees, minus the four execution-control flags (`--background`, `--wait`, `--resume`, `--fresh`) which the agent may strip.
